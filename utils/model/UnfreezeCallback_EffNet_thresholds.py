...
class UnfreezeCallback_EffNet_thresholds(pl.Callback):
    def __init__(self, patience: int = 3, min_delta: float = 0.0):
        ...
        self.thresholds_unfrozen = False
    ...
    def on_validation_end(self, trainer, pl_module):
        ...
        if self.best_val_loss is None:
            ...
            if self.wait >= self.patience and not self.unfrozen_once:
                ...
                print("Unfreezing all layers and learnable thresholds due to plateau in validation loss.")
                self.unfreeze_layers(pl_module, num_layers=None)
                self.unfreeze_thresholds(pl_module)
    ...
    def unfreeze_layers(self, pl_module, num_layers=None):
        ...
        new_optimizer = torch.optim.AdamW([{
            ...
            {'params': pl_module.age_classifier.parameters()},
            {'params': pl_module.learnable_thresholds, 'lr': 1e-2 if self.thresholds_unfrozen else 1e-4}  # Adjust learning rate for thresholds if unfrozen
    ...
    def unfreeze_thresholds(self, pl_module):
        """
        Unfreezes the learnable thresholds to allow them to be optimized.
        """
        print("Unfreezing learnable thresholds.")
        pl_module.learnable_thresholds.requires_grad = True
        self.thresholds_unfrozen = True
