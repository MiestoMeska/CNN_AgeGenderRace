...
def plot_multiple_normalized_landmarks(df, columns, labels=None):
    """
    Function to plot scatterplots of multiple normalized landmark positions in a 3-wide layout.
    ...
    for i, column in enumerate(columns):
        ...
        ax.set_title(f"Scatter plot of normalized {labels[i]} positions")
        ax.set_xlabel("Horizontal position (normalized)")
        ax.set_ylabel("Vertical position (normalized)")
        ax.set_xlim(0, 1)
        ax.set_ylim(0, 1)
        ax.axhline(df[column].apply(lambda x: x[1]).mean(), color='red', linestyle='--', label='Average Y')
        ax.axvline(df[column].apply(lambda x: x[0]).mean(), color='red', linestyle='--', label='Average X')
        ax.grid(True)
        ax.legend()
